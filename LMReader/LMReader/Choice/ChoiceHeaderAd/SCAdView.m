...
@property (nonatomic, assign) NSInteger adTimeCount;
...
-(void)sc_collectioViewScrollToIndex:(NSInteger)index{
    ...
    if (self.delegate &&[self.delegate respondsToSelector:@selector(sc_scrollToIndex:)]) {
        NSInteger realIndex = index%_builder.adArray.count;
        [self.delegate sc_scrollToIndex:realIndex];
    }
}
...
-(void)startCount {
    TopicAd* detailAd = [self.dataArray objectAtIndex:self.currentAdIndex];
    Ad* ad;
    if ([detailAd hasBook]) {
        ad = detailAd.book;
    }else {
